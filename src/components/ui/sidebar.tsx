...
>(
  (
    ...
    // Auto-cierra el sidebar al hacer scroll en cualquier módulo/página
    React.useEffect(() => {
      let ticking = false;

      const handleScroll = (e: Event) => {
        const target = e.target as HTMLElement | null;
        // Si el scroll ocurre dentro del propio sidebar, no lo cerramos
        if (target && target.closest && target.closest("[data-sidebar=sidebar]")) {
          return;
        }
      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            className="w-(--sidebar-width-mobile) max-w-[88vw] bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
            style={
              {
                "--sidebar-width-mobile": SIDEBAR_WIDTH_MOBILE,
              } as React.CSSProperties
  ...
  ({ className, ...props }, ref) => {
    return (
      <main
        ref={ref}
        className={cn(
          "relative flex w-full max-w-full flex-1 flex-col bg-background min-w-0 overflow-x-clip",
